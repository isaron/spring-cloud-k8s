...
#每个 pod 的 EUREKA_URL_LIST 都设置成了全部的 pod 域名。使用的时候，可以自行判断，选择不向自己注册。
#例如 eureka-0 的 EUREKA_URL_LIST 可以剔除 http://eureka-0.eureka-service.default.svc.cluster.local:8761/eureka/
...
echo "start jar...."
java -jar $JAVA_OPTS -Djava.security.egd=file:/dev/./urandom -Dspring.profiles.active=docker /eureka.jar